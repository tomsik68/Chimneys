	...
	public boolean onCommand(CommandSender sender, Command command, String label,
		...
		if(args.length == 0){
			...
			if(args[0].equalsIgnoreCase("help")){
				sender.sendMessage(ChatColor.RED+"/chimney [tgt | std] - Creates chimney at your target block(tgt) or block you're standing on(std)");
			}else if(args[0].equalsIgnoreCase("std") && plugin.perms.getPermissor().has(player, "chimneys.create")){
			...
			}else if(args[0].equalsIgnoreCase("tgt")&& plugin.perms.getPermissor().has(player, "chimneys.create")){
			...
			}else if(args[0].equalsIgnoreCase("wand")&& plugin.perms.getPermissor().has(player, "chimneys.wand.get")){
			...
			if(args[1].equalsIgnoreCase("rs")&& plugin.perms.getPermissor().has(player, "chimneys.create")){
